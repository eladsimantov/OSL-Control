from opensourceleg.sensors.base import IMUBase, EncoderBase, LoadcellBase, ABC
from opensourceleg.sensors.loadcell import LoadcellBrokenWireDetectedException, LoadcellNotRespondingException
...
class SRILoadCell_M8123B2(LoadcellBase):
    """
    Implementation for the Sunrise Instruments 6 axis load cell, with an OEM circuit board (M8123B2).
    The load cell is connected to the OEM board which interfaces with the main controller via CAN. 
    """
...
if __name__ == "__main__":
    # Example usage of the SRILoadCell_M8123B2 class
    load_cell = SRILoadCell_M8123B2(tag="SRILoadCell", offline=True, calibration_matrix=np.eye(6))
    try:
        print(f"Load: {load_cell} N")
    except LoadcellNotRespondingException:
        print("Load cell is not responding.")
    except LoadcellBrokenWireDetectedException:
        print("Broken wire detected in the load cell.")