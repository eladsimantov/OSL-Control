...
class ODriveCAN:
    def __init__(self, bus_name="can0", node_id=1, dbc_path="/home/enable-lab/OSL-Control/src/drivers/odrive-cansimple.dbc"):
        ...
        self.axisID = node_id
        # open CAN and DBC
        self.bus = can.Bus(bus_name, interface="socketcan")
        try:
...
class SingleMotorController:
    ...
    def __init__(self, motor_knee: ODriveMotor,
                 knee_phase_deg=-30.0):
        self.knee = motor_knee
        motor_knee.gear_ratio = 20
        # self.ankle = motor_ankle
    ...
    def set_phase_between(self):
        """Set ankle phase relative to knee (knee phase kept 0)."""
        self.knee_phase = 0.0
        # self.ankle_phase = math.radians(ankle_phase_deg)
        print(f"→ phases set: knee {math.degrees(self.knee_phase):.1f}°")

    def set_phases(self, knee_phase_deg):
        self.knee_phase = math.radians(knee_phase_deg)
        # self.ankle_phase = math.radians(ankle_phase_deg)
        print(f"→ phases set: knee {knee_phase_deg:.1f}°")

    def inc_phases(self, knee_inc_deg=0.0):
        self.knee_phase += math.radians(knee_inc_deg)
        # self.ankle_phase += math.radians(ankle_inc_deg)
        print(f"→ phases incremented: knee {math.degrees(self.knee_phase):.1f}°")
    ...
    def walking_loop(self):
        ...
        while self.walking:
            ...
            if time.time() - last_print > 2.0:
                last_print = time.time()
                print(f"  gait: knee_phase={math.degrees(self.knee_phase):.1f}°")
            time.sleep(self.loop_dt)
        print("→ Walking cycle stopped")
...
class CommandInterfaceSingle:
    def __init__(self, motor1: ODriveMotor, walker: SingleMotorController):
        self.motor1 = motor1
        self.walker = walker
...
def main():
    ...
    walker = SingleMotorController(knee)
    # # walker = DualMotorController(knee, ankle)
    ui = CommandInterfaceSingle(knee, walker)
    ui.print_menu()
    while True:
        try:
